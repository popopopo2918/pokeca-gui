...
class GameView:
    ...
    def public_attack_projections(
        ...
        *,
        ...
        battle_cage = any(
            card is not None and int(card.get("id", -1)) == 1264
            for card in (self.current.get("stadium") or [])
        )
        ...
        for attacker in self.opponent_field:
            ...
            for attack_id in card.attacks:
                ...
                if pressure is not None:
                    ...
                    globally_blocked = uses_counters and battle_cage
                    target_blocked = (
                        not uses_counters
                        and target_meta is not None
                        and target_meta.tera
                    )
                    if not globally_blocked:
                        target_slots = (
                            len(self.bench)
                    ...
                    if not globally_blocked and not target_blocked:
