...
from memory import card_may_be_in_deck, possible_deck_count as _possible_deck_count
from model import Area, OptionType, SelectContext, SelectType
from proposals import PendingIntent, Proposal, covers
from rules.attack import can_current_legal_attack_ko, can_hand_power_ko
...
def possible_deck_count(view, memory, card_id: int | CardId) -> int:
    """自デッキ検索では不採用のふうせんを候補にしない。"""
    if int(card_id) == int(CardId.AIR_BALLOON):
        return 0
    return _possible_deck_count(view, memory, card_id)
...
def rank_full_board_search_candidates(
    view,
    memory,
    plan: FullBoardPlan,
) -> tuple[tuple[int, ...], ...]:
    """盤面下限・対応進化札・既知残数で基本候補を順位group化する。"""

    def key(card_id: int) -> tuple[int, int, int, int]:
        value = int(card_id)
        attack = value == int(CardId.ABRA)
        attack_continuity_needed = attack and needs_attack_line_setup(view)
        floor_deficit = max(
            0,
            MINIMUM_ATTACK_LINES - attack_line_count(view)
            if attack
            else MINIMUM_DRAW_LINES - draw_line_count(view),
        )
        next_stage_ready = (
        ...
        return (
            0 if attack_continuity_needed else 1,
            -floor_deficit,
            0 if next_stage_ready else 1,
            -possible_deck_count(view, memory, value),
        )

    ranked = sorted(
        (int(card_id) for card_id in plan.target_basic_ids),
        key=lambda card_id: (*key(card_id), card_id),
    )
    groups: list[list[int]] = []
    for card_id in ranked:
        if groups and key(groups[-1][0]) == key(card_id):
            groups[-1].append(card_id)
        else:
...
def _item_lock_evolution_priority(view) -> tuple[int, ...] | None:
    """次番の公開グッズロック前に未確保の通常進化経路を優先する。"""
    field_abras = _field_count(view, CardId.ABRA)
    field_kadabras = _field_count(view, CardId.KADABRA)
    required_kadabras = min(2, field_abras + field_kadabras)
    assigned_hand_kadabras = min(
        _hand_count(view, CardId.KADABRA),
        field_abras,
    )
    secured_kadabras = field_kadabras + assigned_hand_kadabras
    if required_kadabras == 0 or secured_kadabras >= required_kadabras:
        return None
...
def _dawn_groups(view, memory) -> tuple[tuple[int, ...], ...]:
    basics = tuple(
        card_id
        for group in rank_full_board_search_candidates(
            view,
            memory,
            full_board_plan(view),
        )
...
def _dawn_attack_route_groups(
    ...
    *,
    ...
    return (
        (
            (int(CardId.ABRA),)
            if attack_line_count(view) < MAXIMUM_ATTACK_LINES
            else ()
        ),
        (
            (int(CardId.KADABRA), int(CardId.DUDUNSPARCE))
...
def _search_groups_have_public_candidate(view, memory, groups) -> bool:
    return any(
...
def _hilda_proposal(
    ...
    if not _search_groups_have_public_candidate(view, memory, groups):
        return None
...
def _dawn_proposal(
    ...
    groups = _dawn_groups(view, memory) if groups is None else groups
...
def _evolution_draw_precedes_non_ko_attach(view, candidates) -> bool:
    candidate_serials = {
        pokemon.serial
        for pokemon in candidates
        if (
            pokemon is not None
            and pokemon.serial is not None
            and int(pokemon.id) in _ATTACKER_IDS
            and not pokemon.appear_this_turn
        )
    }
    return (
        bool(candidate_serials)
        and not can_current_legal_attack_ko(view)
        and any(
            option.type == int(OptionType.ATTACH)
            and option.card_id in _ATTACK_PSYCHIC_IDS
            and option.target is not None
            and option.target.serial in candidate_serials
            for option in view.options
        )
    )
...
def _attack_energy_group(view, target, candy_route: bool) -> tuple[int, ...]:
    ...
    telepath_first = (
        ...
        and attack_line_count(view) < MAXIMUM_ATTACK_LINES
        and bench_space > 0
    )
    if telepath_first:
        return (
...
def propose_search(view, memory) -> Proposal | None:
    ...
    if maturity_needed and not maturity_item_route:
        ...
        if setup_hilda is not None:
            hilda_proposal = _hilda_proposal(
                view,
                memory,
                setup_hilda,
                (
            ...
            if hilda_proposal is not None:
                return hilda_proposal
        if setup_dawn is not None:
            dawn_proposal = _dawn_proposal(
                view,
                memory,
                setup_dawn,
                (
                ...
                groups=_dawn_attack_route_groups(view),
            )
            if dawn_proposal is not None:
                return dawn_proposal
    ...
    if continuity_energy_search:
        ...
        if hilda_proposal is not None:
            return hilda_proposal
    immediate_route_target, immediate_needs_alakazam, immediate_candy_route = (
        _attack_route(view, memory)
    )
    must_search_immediate_candy_alakazam = (
        (hilda is not None or dawn is not None)
        and int(CardId.RARE_CANDY) in view.hand_ids
        and _hand_count(view, CardId.ALAKAZAM) == 0
        and immediate_candy_route
        and immediate_needs_alakazam
        and (
            _has_play_option(view, CardId.RARE_CANDY)
            or (
                immediate_route_target is not None
                and view.has_psychic_energy(immediate_route_target)
            )
        )
    )
    immediate_candy_alakazam_secured = (
        immediate_candy_route
        and (
            not immediate_needs_alakazam
            or must_search_immediate_candy_alakazam
        )
    )
    immediate_powered_kadabra_route = (
        immediate_route_target is not None
        and int(immediate_route_target.id) == int(CardId.KADABRA)
        and not immediate_route_target.appear_this_turn
        and view.has_psychic_energy(immediate_route_target)
        and _can_reach_attack_position_this_turn(view, immediate_route_target)
    )
    item_lock_evolution_group = _item_lock_evolution_priority(view)
    if (
        item_lock_evolution_group is not None
        and not immediate_candy_alakazam_secured
        and not immediate_powered_kadabra_route
    ):
        if hilda is not None:
            hilda_proposal = _hilda_proposal(
                view,
                memory,
                hilda,
                item_lock_evolution_group,
                (
                    int(CardId.BASIC_PSYCHIC),
                    int(CardId.TELEPATH_PSYCHIC_ENERGY),
                    int(CardId.ENRICHING_ENERGY),
                ),
                "公開された次番のグッズロックに備え、トウコで通常進化用ユンゲラーを確保する",
            ...
            if hilda_proposal is not None:
                return hilda_proposal
        if dawn is not None:
            dawn_groups = _dawn_groups(view, memory)
            dawn_proposal = _dawn_proposal(
                view,
                memory,
                dawn,
                965,
                "公開された次番のグッズロックに備え、ヒカリで通常進化用ユンゲラーを確保する",
                extra_rule_ids=("PLAYBOOK-T3-KADABRA",),
                groups=(
                    dawn_groups[0],
                    (
    ...
    if _field_attack_line_is_empty(view):
        if dawn is not None:
            ...
            if dawn_proposal is not None:
                return dawn_proposal
        from rules.petrel import first_turn_petrel_places_missing_attack_line

        petrel_places_first_abra = (
            first_turn_petrel_places_missing_attack_line(view, memory)
        )
        if (
            hilda is not None
            and _old_field_count(view, CardId.DUNSPARCE) > 0
            and not petrel_places_first_abra
        ):
            hilda_proposal = _hilda_proposal(
                view,
                memory,
    ...
    if (
        ...
        dawn_proposal = _dawn_proposal(
            ...
            dawn,
            936
            if _evolution_draw_precedes_non_ko_attach(view, (route_target,))
            else 875,
    ...
    safe_fresh_powered_kadabra_waits = any(
        int(pokemon.id) == int(CardId.KADABRA)
        and pokemon.appear_this_turn
    ...
    if (
        ...
        dawn_proposal = _dawn_proposal(
            ...
            dawn,
            936
            if _evolution_draw_precedes_non_ko_attach(view, old_abras)
            else 875,
            (
    ...
    if (
        ...
        and view.own_turn_number == 2
        and old_abras
        and not safe_fresh_powered_kadabra_waits
    ):
    ...
    dawn_searches_immediate_draw = (
        not attack_psychic_secured
        and view.own_turn_number >= 3
        and dawn_draw_engine is not None
        and card_may_be_in_deck(view, memory, dawn_draw_engine)
    )
    candy_visible = int(CardId.RARE_CANDY) in view.hand_ids
    turn_one_dawn_line_expansion = (
        dawn is not None
        and view.own_turn_number == 1
        and candy_visible
        and attack_line_count(view) == 1
        and any(
    ...
    # ヒカリは1進化と2進化を同時に取れる。今すぐドロー進化を使うなら、
    # ノココッチ／ユンゲラーと将来用フーディンを同時に確保できるため、
    # 「最後のフーディンサーチ手段」として温存する必要はない。
    dawn_to_play = (
        dawn
        if dawn_searches_immediate_draw or turn_one_dawn_line_expansion
        else setup_dawn
    )
    turn_one_dawn_compression = (
        view.own_turn_number == 1 and not candy_visible
    )
    if dawn_to_play is None or (
        not attack_psychic_secured
        and not turn_three_preparation
        and not dawn_extends_unpowered_route
        and not dawn_searches_immediate_draw
        and not turn_one_dawn_compression
        and not turn_one_dawn_line_expansion
    ):
        return None
    if not (
        candy_visible
        or turn_one_dawn_compression
        or turn_one_dawn_line_expansion
        or view.own_turn_number >= 3
        or _can_prepare_turn_three(view)
    ):
        return None
    evolution_draw_precedes_non_ko_attach = (
        dawn_draw_engine in (
            int(CardId.DUDUNSPARCE),
            int(CardId.KADABRA),
        )
        and _evolution_draw_precedes_non_ko_attach(
            view,
            view.field,
        )
    )
    dawn_proposal = _dawn_proposal(
        ...
        dawn_to_play,
        936
        if evolution_draw_precedes_non_ko_attach
        else 795
        if turn_one_dawn_compression or turn_one_dawn_line_expansion
        else 875,
        (
            "1ターン目にアメを温存してヒカリで次のケーシィ系統を展開し、2ターン目以降の攻撃をつなぐ"
            if turn_one_dawn_line_expansion
            else "1ターン目にヒカリでたね・1進化・2進化を山札から抜き、ふしぎなアメの後続ドロー率を上げる"
            if turn_one_dawn_compression
            else "非確定KOの手貼り前にヒカリから進化時ドローへ確定接続し、ドロー結果を見て攻撃役を決める"
            if evolution_draw_precedes_non_ko_attach
            else "フーディンの攻撃用超がないため、ヒカリでノココッチを確保して進化時の追加3枚ドローから探す"
            if dawn_draw_engine == int(CardId.DUDUNSPARCE)
            else "攻撃用超がないため、ヒカリでユンゲラーを確保して2枚ドローから探す"
            if dawn_searches_immediate_draw
            else (
        ...
        if (
            dawn_extends_unpowered_route
            or dawn_searches_immediate_draw
            or turn_one_dawn_compression
            or turn_one_dawn_line_expansion
            or evolution_draw_precedes_non_ko_attach
        )
        else (),
    )
    if dawn_proposal is not None:
