...
from model import Area, LegalOption, OptionType, SelectContext
from proposals import Proposal, covers
from rules.attack import (
    _active_can_complete_hand_power_this_turn,
    _is_hand_power_effect_immune,
    can_hand_power_ko,
    ready_bench_alakazam_can_ko,
)
...
from rules.disruption import should_hold_dudunsparce_ability
...
CURRENT_KO_RICH_DRAW_PRIORITY = 1100
...
def _powered_attacker_can_complete_this_turn(view) -> bool:
    return _active_can_complete_hand_power_this_turn(view)


@covers("PLAYBOOK-ENRICHING-CONDITION", "PLAYBOOK-MAX-DRAW")
def _hand_power_route_can_ko_without_rich(view) -> bool:
    active = view.active
    if active is None:
        return False
    if int(active.id) == int(CardId.ALAKAZAM):
        return can_hand_power_ko(view)
    if not _active_can_complete_hand_power_this_turn(view):
        return False
    ...
    if evolution_hand_cost is None:
        return False
    drawn = min(3, max(0, int(view.own.get("deckCount", 0))))
    projected_hand = max(0, int(view.hand_size) - evolution_hand_cost) + drawn
    return projected_hand >= int(needed)
...
def _active_immediate_alakazam_route_needs_psychic(view) -> bool:
    active = view.active
    if (
        active is None
...
def _opening_pivot_needs_attachment(view, memory) -> bool:
    active = view.active
...
def _bench_alakazam_psychic_attach_completes_this_turn(view) -> bool:
    active = view.active
    if active is None:
        return False
    can_promote = any(
...
def _replay_87270346_turn_one_rich_attach_options(
    view,
) -> tuple[LegalOption, ...]:
    active = view.active
    if (
        ...
        or int(CardId.DAWN) not in view.hand_ids
        or not any(
            option.type == int(OptionType.PLAY)
        ...
        or not any(
            int(pokemon.id) in _ATTACKER_IDS
            for pokemon in view.bench
        )
        or not any(
            option.type == int(OptionType.EVOLVE)
...
def legal_rich_attach_options(view, memory):
    if _opening_pivot_needs_attachment(view, memory):
        return ()
    if _active_immediate_alakazam_route_needs_psychic(view):
        return ()
    if _turn_three_immediate_alakazam_routes_need_energy(view):
        return ()
    replay_turn_one_options = _replay_87270346_turn_one_rich_attach_options(view)
    if replay_turn_one_options:
        return replay_turn_one_options
    if (
        not _powered_attacker_exists(view, memory)
        and not _turn_two_rich_recovery_is_safe(view, memory)
    ):
        return ()
    if _bench_alakazam_psychic_attach_completes_this_turn(view):
        return ()
    evolvable_serials = {
    ...
    usable_dudunsparce_serials = {
        option.source.serial
        for option in view.options
        if (
            option.type == int(OptionType.ABILITY)
            and option.card_id == int(CardId.DUDUNSPARCE)
    ...
    return tuple(
        ...
        if (
            ...
            and (
                (
                    option.target.id == int(CardId.DUNSPARCE)
                    and option.target.serial in evolvable_serials
                    and not option.target.appear_this_turn
                )
                or (
                    option.target.id == int(CardId.DUDUNSPARCE)
                    and option.target.serial in usable_dudunsparce_serials
                )
            )
        )
    )
...
def _old_evolvable_attacker_has_usable_psychic_attach(view) -> bool:
    ...
    return bool(evolvable_serials) and any(
        option.type == int(OptionType.ATTACH)
        and option.card_id in (
            int(CardId.BASIC_PSYCHIC),
            int(CardId.TELEPATH_PSYCHIC_ENERGY),
...
def _incomplete_setup_rich_attach_options(
    view,
    memory,
) -> tuple[LegalOption, ...]:
    if view.own_turn_number < 2:
    ...
    if _turn_three_immediate_alakazam_routes_need_energy(view):
        return ()
    if _hand_power_route_can_ko_without_rich(view):
        return ()
    if _bench_alakazam_psychic_attach_completes_this_turn(view):
        return ()
    if _old_evolvable_attacker_has_usable_psychic_attach(view):
        return ()
    ...
    return tuple(
        ...
        if (
            reserved_serial is None
            or option.target.serial != reserved_serial
            or int(option.target.id) == int(CardId.DUNSPARCE)
        )
    )
...
def _rich_sort_key(option):
    ...
    }.get(-1 if target is None else int(target.id), 5)
...
@covers("PLAYBOOK-ENRICHING-CONDITION", "PLAYBOOK-MAX-DRAW")
def _rich_draw_advances_current_ko(view) -> bool:
    needed = view.required_hand_for_active_ko
    if needed is None or int(view.hand_size) >= int(needed):
        return False
    if (
        not can_hand_power_ko(view, hand_size=int(needed))
        or not can_hand_power_ko(
            view,
            hand_size=int(needed),
            require_legal_option=False,
        )
    ):
        return False
    drawn = min(4, max(0, int(view.own.get("deckCount", 0))))
    projected_hand = max(0, int(view.hand_size) - 1 + drawn)
    return projected_hand > int(view.hand_size)
...
def _active_dudunsparce_deck_floor_ko_is_safe(view, option, draw_count: int) -> bool:
    """前を空けた確定KOを反映し、終盤のにげあしドローを再評価する。"""
    if not _active_dudunsparce_opens_ready_alakazam(view, option):
        return False
    returned = returned_dudunsparce_cards(option)
    available = max(0, int(view.own.get("deckCount", 0))) + returned
    actual_draw = min(max(0, int(draw_count)), available)
    if not ready_bench_alakazam_can_ko(view, view.hand_size + actual_draw):
        return False
    target = view.opponent_active
    meta = None if target is None else view.catalog.card(target.id)
    prizes_taken = 1 if meta is None else max(1, int(meta.prize_value))
    return (
        available - actual_draw
        >= minimum_deck_reserve_after_prizes(view, prizes_taken)
    )
...
def propose_draw(view, memory) -> Proposal | None:
    ...
    if not rich_options:
        rich_options = tuple(
            option
            for option in _incomplete_setup_rich_attach_options(view, memory)
            if can_remove_from_deck(
                view,
                cards_removed=4,
                hand_delta=(
        ...
        incomplete_setup = bool(rich_options)
    if rich_options:
        option = min(rich_options, key=_rich_sort_key)
        target_serial = None if option.target is None else option.target.serial
        replay_turn_one_setup = bool(
            _replay_87270346_turn_one_rich_attach_options(view)
        )
        advances_current_ko = _rich_draw_advances_current_ko(view)
        opening_completion_draws_remain = (
            is_opening_attack_phase(view)
            and view.own_turn_number == 2
            and _powered_attacker_exists(view, memory)
            and not _powered_attacker_can_complete_this_turn(view)
        )
        reuses_existing_dudunsparce = (
            option.target is not None
            and int(option.target.id) == int(CardId.DUDUNSPARCE)
        )
        if advances_current_ko:
            reason = "リッチの正味3枚ドローで現在のハンドパワーをKO打点へ近づける"
        elif replay_turn_one_setup:
            reason = "1ターン目に前のノコッチへリッチを貼って今ドローし、2ターン目の手貼りを攻撃役へ残す"
        elif incomplete_setup:
            reason = "攻撃経路が未完成なので、即時再利用できなくてもリッチで4枚引く"
        elif opening_completion_draws_remain:
            reason = "2ターン目は進化ドローで攻撃成立を確認してからリッチをノコッチへ付ける"
        elif reuses_existing_dudunsparce:
            reason = "攻撃役の超エネルギーを確保済みなのでリッチをノココッチへ付け、にげあしドローで山札へ戻して再利用する"
        else:
            reason = "攻撃役の超エネルギーを確保済みなのでリッチをノコッチへ付け、ノココッチ化後に山札へ戻して再利用する"
        rule_ids = (
            (
                "PLAYBOOK-ENRICHING-CONDITION",
                "PLAYBOOK-NO-ENRICHING-RETREAT",
        ...
        return Proposal(
            (option.position,),
            (
                CURRENT_KO_RICH_DRAW_PRIORITY
                if advances_current_ko
                else 840
                if opening_completion_draws_remain
                else 920
            ),
            reason,
            rule_ids,
    ...
    raw_dudunsparce_options = tuple(
        option for option in _dudunsparce_options(view)
        if not should_hold_dudunsparce_ability(view, memory, option)
    )
    ...
    dudunsparce_options = tuple(
        ...
        if (
            ...
            or _active_dudunsparce_deck_floor_ko_is_safe(
                view,
                option,
                dudunsparce_draw,
            )
    ...
    if dudunsparce_options:
        option = min(
            ...
            key=lambda candidate: (
                0
                if _active_dudunsparce_opens_ready_alakazam(view, candidate)
                else 1,
                *_dudunsparce_sort_key(candidate),
            ),
        )
        has_rich = (
            option.source is not None
        ...
        deck_floor_ko_pivot = (
            ...
            and _active_dudunsparce_deck_floor_ko_is_safe(
                view,
                option,
                dudunsparce_draw,
            )
        ...
        if deck_floor_ko_pivot:
            reason = (
                "山札最低枚数をKO後の残りサイドで再計算し、バトル場の"
                "ノココッチを戻して前を空け、超付きフーディンを出す"
            )
            rule_ids = (
                "PLAYBOOK-DECK-SAFETY",
                "PLAYBOOK-MAX-DRAW",
        ...
        return Proposal(
            ...
            (
                1200
                if deck_floor_ko_pivot or deck_floor_recycle
