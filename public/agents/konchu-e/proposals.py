...
_EFFECTLESS_INTENT_KINDS = frozenset({"SURVIVAL_RETREAT"})
...
class PendingIntent:
    ...
    def deferred_main_from_view(
        ...
        *,
        ...
        """効果解決後、直後のMAINで同じserialだけに渡す意図を作る。"""
    ...
    def effectless_from_view(
        ...
        *,
        ...
        """にげる等、effectカードを持たない連続promptの対象を予約する。"""
        if kind not in _EFFECTLESS_INTENT_KINDS:
            raise ValueError(f"未登録のeffectless intentです: {kind}")
        contexts = tuple(int(context) for context in remaining_contexts)
        if not contexts:
            raise ValueError("effectless intentには選択contextが必要です。")
        return cls(
            kind=kind,
    ...
    def has_complete_origin(self) -> bool:
        has_origin = (
            self.kind in _EFFECTLESS_INTENT_KINDS
            or (
    ...
    def matches_deferred_main(self, view) -> bool:
        """直後のMAINに、発生元として記録したカードserialがあるか照合する。"""
...
class Proposal:
    ...
    deferred_intent: PendingIntent | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.deferred_intent is not None and (
            update is not IntentUpdate.CLEAR
            or not self.deferred_intent.is_deferred_main
        ):
            raise ValueError("deferred intentは完了した意図からMAINへだけ引き継げます。")
