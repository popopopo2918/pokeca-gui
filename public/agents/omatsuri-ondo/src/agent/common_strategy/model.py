...
class PokemonRef(CardRef):
    ...
    def has_energy_type(
        ...
        energy_type: int,
        *,
        compatible_card_ids: tuple[int, ...] = (),
    ) -> bool:
        return (
            int(energy_type) in tuple(int(value) for value in self.energies)
            or any(
                int(card_id) in tuple(
                    int(value) for value in compatible_card_ids
                )
                for card_id in self.energy_card_ids
            )
        )
...
@dataclass(frozen=True)
class PublicAttackProjection:
    attacker_serial: int | None
    attack_id: int
    active_damage: int
    fixed_bench_damage: int
    bench_damage_budget: int
    bench_target_limit: int | None
    bench_damage_distributable: bool
...
class GameView:
    ...
    def from_observation(
        ...
        current = sanitized_raw["current"]
        own_index = int(current["yourIndex"])
        select = sanitized_raw.get("select") or {}
        catalog = catalog or CardCatalog.empty()
        players = current["players"]
        opponent = players[1 - own_index]
        for area_name in ("hand", "deck", "prize"):
            cards = opponent.get(area_name)
            if isinstance(cards, list):
                opponent[area_name] = [None] * len(cards)
            elif cards is not None:
                opponent[area_name] = None
        opponent_deck_selected = False
        for option in select.get("option") or []:
            owner = int(option.get("playerIndex", own_index))
            if (
                owner != own_index
                and cls._is_hidden_card_area(option.get("area"))
            ):
                opponent_deck_selected = (
                    opponent_deck_selected
                    or int(option["area"]) == int(Area.DECK)
                )
                option.pop("cardId", None)
                option.pop("serial", None)
        if opponent_deck_selected:
            deck = select.get("deck")
            if isinstance(deck, list):
                select["deck"] = [None] * len(deck)
            elif deck is not None:
                select["deck"] = None
        shell = cls(
            sanitized_raw, select, current, own_index, players[own_index],
            opponent, (), catalog,
        )
        options = tuple(
    ...
    @property
    def public_stadium_ids(self) -> frozenset[int]:
        return frozenset(
            int(card["id"])
    ...
    def complete_known_deck_ids(self) -> tuple[int, ...] | None:
        ...
        if (
            deck is None
            or len(deck) != int(self.own.get("deckCount", 0))
            or any(card is None for card in deck)
        ):
            return None
        return tuple(int(card["id"]) for card in deck)
    ...
    def public_attack_is_ready(
        self,
        pokemon: PokemonRef | None,
        attack_id: int,
    ) -> bool:
        """公開済みエネルギーだけで固定コストのワザを使えるか判定する。"""
        if pokemon is None:
            return False
        card = self.catalog.card(pokemon.id)
        meta = self.catalog.attack_meta.get(int(attack_id))
        if (
            meta is None
            or card is None
            or int(attack_id) not in tuple(int(value) for value in card.attacks)
        ):
            return False
        available = Counter(int(energy) for energy in pokemon.energies)
        rainbow = available.pop(int(EnergyType.RAINBOW), 0)
        colorless = 0
        for required in meta.energies:
            required = int(required)
            if required == int(EnergyType.COLORLESS):
                colorless += 1
                continue
            if available[required] > 0:
                available[required] -= 1
            elif (
                required in (int(EnergyType.PSYCHIC), int(EnergyType.DARKNESS))
                and available[int(EnergyType.TEAM_ROCKET)] > 0
            ):
                available[int(EnergyType.TEAM_ROCKET)] -= 1
            elif rainbow > 0:
                rainbow -= 1
            else:
                return False
        return sum(available.values()) + rainbow >= colorless

    def public_attack_projections(
        self,
        bench_target: PokemonRef | None = None,
        *,
        excluded_attacker_serials: frozenset[int] = frozenset(),
    ) -> tuple[PublicAttackProjection, ...]:
        """公開済みで現在使用可能な相手ワザを、ワザ単位の打点へ投影する。"""
        target_meta = (
            None
            if bench_target is None
            else self.catalog.card(bench_target.id)
        )
        projections: list[PublicAttackProjection] = []
        for attacker in self.opponent_field:
            if (
                attacker.serial is not None
                and int(attacker.serial) in excluded_attacker_serials
            ):
                continue
            card = self.catalog.card(attacker.id)
            if card is None:
                continue
            for attack_id in card.attacks:
                if not self.public_attack_is_ready(attacker, attack_id):
                    continue
                attack = self.catalog.attack_meta.get(int(attack_id))
                if attack is None:
                    continue
                fixed_bench_damage = 0
                bench_damage_budget = 0
                bench_target_limit: int | None = 0
                bench_damage_distributable = False
                pressure = self._fixed_bench_pressure(attack.text)
                if pressure is not None:
                    damage, uses_counters = pressure
                    bench_damage_distributable = bool(
                        uses_counters
                        and self._is_distributable_bench_counter_text(attack.text)
                    )
                    bench_target_limit = (
                        None
                        if bench_damage_distributable
                        else self._bench_target_limit(attack.text)
                    )
                    target_blocked = (
                        not uses_counters
                        and target_meta is not None
                        and target_meta.tera
                    )
                    target_slots = (
                        len(self.bench)
                        if bench_target_limit is None
                        else max(0, int(bench_target_limit))
                    )
                    bench_damage_budget = (
                        max(0, int(damage))
                        if bench_damage_distributable
                        else max(0, int(damage)) * target_slots
                    )
                    if not target_blocked:
                        fixed_bench_damage = max(0, int(damage))
                projections.append(PublicAttackProjection(
                    attacker_serial=attacker.serial,
                    attack_id=int(attack_id),
                    active_damage=max(0, int(attack.damage)),
                    fixed_bench_damage=fixed_bench_damage,
                    bench_damage_budget=bench_damage_budget,
                    bench_target_limit=bench_target_limit,
                    bench_damage_distributable=bench_damage_distributable,
                ))
    ...
    def max_public_bench_pressure(
        ...
        *,
        ...
        """対象へ同一の公開ワザが与える固定ベンチ打点の最大値を返す。"""
        if (
            pokemon is None
            or pokemon.player_index != self.own_index
            or int(pokemon.area or -1) != int(Area.BENCH)
        ):
            return 0
        return max(
            (
                projection.fixed_bench_damage
                for projection in self.public_attack_projections(
                    pokemon,
    ...
    def opponent_can_public_item_lock_next_turn(
        self,
        *,
        excluded_attacker_serials: frozenset[int] = frozenset(),
    ) -> bool:
        """公開済みの使用可能ワザが次の自分番をグッズロックするか返す。"""
        return any(
            attack is not None
            and self._is_next_turn_item_lock_text(attack.text)
            for projection in self.public_attack_projections(
                excluded_attacker_serials=excluded_attacker_serials,
            )
            for attack in (self.catalog.attack_meta.get(projection.attack_id),)
        )

    def public_bench_damage_risk(
        self,
        pokemon: PokemonRef | None,
        *,
        excluded_attacker_serials: frozenset[int] = frozenset(),
    ) -> bool:
        """公開済みの使用可能ワザだけで対象ベンチが次番に倒れるか返す。"""
        return (
            pokemon is not None
            and self.max_public_bench_pressure(
                pokemon,
                excluded_attacker_serials=excluded_attacker_serials,
            ) >= int(pokemon.hp)
        )
    ...
    def _resolve_option(self, position: int, option: dict[str, Any]) -> LegalOption:
        ...
        hidden_opponent_card = (
            owner != self.own_index
            and self._is_hidden_card_area(option.get("area"))
        )
        source = self._pokemon_for_area(option.get("area"), option.get("index"), owner)
        raw_card = (
            None
            if hidden_opponent_card
            else self._raw_card_for_option(option, source)
        )
        card_id = None
        card_serial = None
        ...
        elif not hidden_opponent_card and option.get("cardId") is not None:
    ...
    def _card_at_area(self, area: int | None, index: int | None, player: int) -> dict[str, Any] | None:
        ...
        area_value = int(area)
        if (
            player != self.own_index
            and self._is_hidden_card_area(area_value)
        ):
            return None
    ...
    @staticmethod
    def _is_hidden_card_area(area: int | None) -> bool:
        if area is None:
            return False
        return int(area) in (
            int(Area.HAND),
    ...
    @staticmethod
    def _is_next_turn_item_lock_text(text: str) -> bool:
        lower = text.lower().replace("’", "'")
        english_next_turn = (
            "during your opponent's next turn" in lower
            or (
                "your opponent" in lower
                and "during their next turn" in lower
            )
        )
        english_lock = re.search(
            r"\b(?:can't|cannot|may not)\s+play\s+(?:any\s+)?item cards?\b"
            r"[^.]*\bfrom\s+(?:their|the)\s+hand\b",
            lower,
        )
        japanese_lock = (
            "次の相手の番" in text
        ...
        return bool(
            english_next_turn and english_lock is not None
            or japanese_lock
        )

    @staticmethod
    def _fixed_bench_pressure(text: str) -> tuple[int, bool] | None:
        lower = text.lower().replace("’", "'")
        if GameView._is_bench_counter_text(text):
            match = re.search(r"\b(\d+)\s+damage counters?\b", lower)
            if match is not None:
                return int(match.group(1)) * 10, True
        if GameView._is_bench_damage_text(text):
            match = re.search(r"\b(\d+)\s+damage\s+to\b", lower)
            if match is not None:
                return int(match.group(1)), False
        return None

    @staticmethod
    def _bench_target_limit(text: str) -> int | None:
        lower = text.lower().replace("’", "'")
        explicit = re.search(
            r"\b(?:choose\s+)?(\d+)\s+of\s+your opponent's "
            r"(?:benched\s+)?(?:pokemon|pokémon)\b",
            lower,
        )
        if explicit is not None:
            return max(1, int(explicit.group(1)))
        if re.search(
            r"\beach of your opponent's benched (?:pokemon|pokémon)\b",
            lower,
        ):
            return None
        return 1
    ...
    @staticmethod
    def _is_distributable_bench_counter_text(text: str) -> bool:
        lower = text.lower().replace("’", "'")
        return bool(
            GameView._is_bench_counter_text(text)
            and re.search(r"\bin any way you like\b", lower)
        )
